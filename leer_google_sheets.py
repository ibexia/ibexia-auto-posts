...
<p><strong>{data['NOMBRE_EMPRESA']} ({data['TICKER']})</strong> cotiza actualmente a <strong>{data['PRECIO_ACTUAL']:,}€</strong>, un nivel que considero estratégico. Mi precio objetivo de compra se sitúa en <strong>{data['PRECIO_OBJETIVO_COMPRA']:,}€</strong>. <p>Este último gráfico es clave para identificar posibles puntos de reversión en función de tu estrategia. Para entender su funcionamiento e interpretación, puedes <a href=\"https://ibexia.es/como_funciona\" target=\"_blank\" style=\"color: #007BFF; text-decoration: none;\">hacer clic aquí</a>.</p>
