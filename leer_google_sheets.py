...
def obtener_datos_yfinance(ticker):
    try:
        ...
        # Se mantiene el cálculo de fechas de proyección para llenar la serie de datos
        ...
        if len(ohlc_history_full) >= 30:
            ...
            for idx, row in ohlc_history_for_chart.iterrows():
                ...
                # Se usará la fecha en formato ISO y luego se ajustará a 'category' en JS
        ...
        # SE ELIMINA DE LA LOGICA DEL GRÁFICO FINAL (Punto CUARTO), pero se mantiene la lista por si el código la usa en otro sitio.
...
def construir_prompt_formateado(data):
    ...
    # La variable cierres_reales_linea ya no se usa en el gráfico, pero se mantiene para consistencia.
    ...
    if not ohlc_data or not smi_data or num_labels_total == 0:
        ...
        for i, date_str in enumerate(fechas_completo):
             # Usamos solo la fecha en formato string para el eje 'category'
        ...
        # 4. Etiquetas de Categoría (Fechas en formato string)
        fechas_json = json.dumps(fechas_completo)
        ...
        # --- APEXCHARTS: CANDLESTICK CON LÍNEA SMI Y PROYECCIÓN (AHORA EN EL MISMO GRÁFICO) ---
        chart_html = f"""
        <script src="https://cdn.jsdelivr.net/npm/apexcharts"></script>
        <div id="chartCandlestickSMI" style="width: 100%; max-width: 800px; margin: auto; background-color: #1a1a2e; padding: 20px; border-radius: 10px;"></div>
        ...
        var fechasCompleto = {fechas_json}; // Etiquetas de Categoría
        
        // El SMI y el Precio estarán en la misma ventana, el SMI en un eje Y secundario.
        ...
        var optionsCandlestickSMI = {{
            series: [
                {{
                    ...
                    data: smiData,
                    color: '#00bfa5',
                    yAxisId: 'smiAxis', // Asignar al eje del Algoritmo
                }},
                {{
                    name: 'Precio Real',
                    type: 'candlestick',
                ...
                {{
                    ...
                    yAxisId: 'priceAxis', // Asignar al eje de Precio
                    stroke: {{
                        dashArray: 5, // QUINTO REQUISITO: Línea a trazos
                        width: 0.5 // PRIMER REQUISITO: Grosor al mínimo
                    }},
                    marker: {{
                        size: 0
                    }}
            ...
            title: {{
                text: 'Gráfico de Precio, Proyección y Algoritmo SMI',
                align: 'left',
                style: {{
                    color: '#e0e0e0'
                }}
            }},
            // SEGUNDO REQUISITO: Cambiar a 'category' y usar las fechas como etiquetas
            xaxis: {{
                type: 'category',
                categories: fechasCompleto, 
                tooltip: {{
                    enabled: true
                }},
                labels: {{
                    formatter: function(val) {{
                         if (val) {{
                            // Formato de fecha DD MMM (ej. 14 Nov)
            ...
            // TERCER REQUISITO: Ejes Y combinados
            yaxis: [
                {{ // Eje Y para el Algoritmo (SMI)
                    seriesName: 'Nuestro Algoritmo (SMI)',
                    min: -100,
                    max: 100,
                    tickAmount: 8,
                    title: {{
                    ...
                    opposite: false, // Eje SMI a la izquierda
                    axisBorder: {{
                        show: true,
                        color: '#00bfa5'
                    }},
                    ...
                    id: 'smiAxis',
                    tooltip: {{
                        enabled: false // No mostrar tooltip doble
                    }},
                    annotations: {{
                        yaxis: [
                            {{
                ...
                {{ // Eje Y para el Precio (Candlestick y Proyección)
                    seriesName: 'Precio Real',
                    title: {{
                        text: 'Precio (EUR)',
                        style: {{
                            color: '#e0e0e0'
            ...
            plotOptions: {{
                candlestick: {{
                    ...
                    // PRIMER REQUISITO: Grosor de las velas al mínimo
                    boxShadow: {{
                        enabled: false // Para reducir la apariencia de negrita
                    }},
                    // Las velas de ApexCharts ya son finas, esto ayuda a la línea del contorno
                    border: {{ 
                        width: 0.5 
                    }}
                }}
            }},
            stroke: {{ // PRIMER REQUISITO: Grosor de las líneas al mínimo (para SMI y Proyección)
                 width: [0.5, 0.5, 0.5] // SMI, Candlestick(no aplica), Proyección
            }},
            tooltip: {{
                theme: 'dark',
                x: {{
                    formatter: function(val) {{
                        return val; // El valor ya es la fecha string (YYYY-MM-DD)
                    }}
                }}
            }},
            grid: {{
        ...
        var chartCandlestickSMI = new ApexCharts(document.querySelector("#chartCandlestickSMI"), optionsCandlestickSMI);
        chartCandlestickSMI.render();
        
        </script>
        """ # Se eliminó el segundo chart y se combinaron los scripts.
